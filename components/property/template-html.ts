...
export const buildPropertyLandingHtml = ({
  ...
  const mapAccent = hexToRgba(palette.colors.accent, isDarkPalette ? 0.25 : 0.2);
  const mapAccentAlt = hexToRgba(
    palette.colors.accentStrong,
    isDarkPalette ? 0.2 : 0.15
  );
  const modalBg = isDarkPalette ? "#0b0f12" : "#0f1418";
  ...
  const heroActionsHtml = `
    <div class="hero-actions">
      ...
      <a class="btn btn-outline" href="#galleria" data-scroll>${escapeHtml(
        heroSecondaryCta
      )}</a>
    </div>
  `;
  ...
  const contactFormHtml = contactEmail
    ? `
        <form class="contact-form" data-contact-form data-contact-email="${escapeHtml(
          ...
          <label>
            <span>Messaggio</span>
            <textarea name="message" rows="4" placeholder="Scrivi qui..."></textarea>
          </label>
    ...
    <script>
      ...
      contactForms.forEach((form) => {
        form.addEventListener("submit", (event) => {
          ...
          const body = ["Nome: " + name, "Email: " + from, "", message]
            .join("\\n")
            .trim();
          const href =
            "mailto:" +
